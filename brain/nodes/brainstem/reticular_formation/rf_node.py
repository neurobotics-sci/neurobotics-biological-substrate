"""brain/nodes/brainstem/reticular_formation — v10.17"""
...
class ReticuloSpinalNode:
    ...
    def start(self):
        self.bus.start();self.bus.subscribe(T.AMYG_CEA_OUT,self._on_cea);self.bus.subscribe(T.CTX_PFC_CMD,self._on_pfc)
        self._running=True;logger.info(f"{self.name} v10.17 | arousal | MLR")
    def stop(self): self._running=False;self.bus.stop()

if __name__=="__main__":
    with open("/etc/brain/config.json") as f: cfg=json.load(f).get("reticular_formation",{"pub_port":5660,"sub_endpoints":[]})
