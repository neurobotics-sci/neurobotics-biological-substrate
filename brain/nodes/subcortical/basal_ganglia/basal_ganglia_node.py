"""
brain/nodes/subcortical/basal_ganglia/basal_ganglia_node.py
Basal Ganglia: DA-modulated action selection + ACC conflict monitoring (absorbed from cingulate).

Conflict monitoring (ACC function) absorbed here from removed cingulate Orin node.
...
class BasalGangliaNode:
    ...
    def _loop(self):
        iv = 1.0 / self.HZ
        while self._running:
            #logging.info("🧠 Basal Ganglia Reporting For Duty!")
            t0 = time.time()
            #self.wd.heartbeat()
            result = self._select()
            now_ns = time.time_ns()
    ...
    def start(self):
        ...
        logger.info(f"{self.name} Bubo Sapiens | DA-temp | ACC-conflict(absorbed) | {self.HZ}Hz")
