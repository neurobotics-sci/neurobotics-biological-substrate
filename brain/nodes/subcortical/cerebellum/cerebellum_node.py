"""
brain/nodes/subcortical/cerebellum/cerebellum_node.py — Bubo Mark XVI Chassis
...
from brain.nodes.subcortical.cerebellum.cmac_cerebellum import CMACCerebellumNode
from brain.balance.mpc.mpc_balance_controller import MPCBalanceController
from brain.rl.gait_rl.ppo_gait_learner import PPOGaitLearner
...
class CerebellumNode:
    ...
    def start(self):
        ...
        logger.info(f"{self.name} Bubo Cerebellum | CMAC + MPC + PPO | {self.HZ}Hz")
...
if __name__ == "__main__":
    with open("/etc/brain/cerebellum/config.json") as f: cfg=json.load(f) 
    n = CerebellumNode(cfg); n.start()
    try:
