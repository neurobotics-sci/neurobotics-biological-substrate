"""
brain/safety/diagnostics/body_integrity.py — Brain v10000
...
In Brain, the Body Integrity Monitor plays the Insula role:
...
AUTOPOIESIS CONTROLLER:
  ...
  5. Generate repair request (repair_request.txt in /opt/brain/data/)
...
from brain.safety.diagnostics.fault_model import (
    Fault, FaultSeverity, DiagnosticReport, REGION_ZONES
)
from brain.safety.diagnostics.self_diagnostic import SelfDiagnosticEngine
from brain.safety.diagnostics.protective_postures import ProtectivePostureController
from brain.safety.diagnostics.fault_communicator import FaultCommunicator
from brain.bus.neural_bus import NeuralBus, T

logger = logging.getLogger("BodyIntegrity")

REPAIR_REQUEST_PATH = Path("/opt/brain/data/repair_requests.json")
BODY_STATE_PATH     = Path("/opt/brain/data/body_state.json")
...
class BodyIntegrityMonitor:
    ...
    def _generate_repair_request(self, fault: Fault):
        ...
        request = {
            ...
            "brain_status":     f"Body integrity at {self._body_integrity:.0%}",
        }
    ...
    def _log_damage_context(self, fault: Fault):
        ...
        Brain should remember what it was doing when it got hurt.
        """
        if not self._bus: return
