"""
brain/safety/diagnostics/fault_communicator.py — Brain v10000

Fault Communicator: Telling Humans What's Wrong, and How to Help
=================================================================

The social layer of the self-diagnostic system.
...
Brain does the same. The Fault Communicator is the bridge between
the technical fault model and the social interaction layer.

KEY PRINCIPLES:
  1. DIGNITY: Brain does not panic. It reports calmly, clearly,
  ...
  4. REPAIR GUIDANCE: Every fault report includes what the human
     can do to help. Brain is not just reporting — it's asking
     for help in a specific, actionable way.

  5. EMOTIONAL HONESTY: The EmotionChip receives a pain/discomfort
     signal from active faults. Brain's voice will carry that.
     Not dramatically — but honestly.

  6. AMYGDALA LEARNING: After communicating a fault, the social
     node logs the context (what was Brain doing, where, with whom)
     so the amygdala can encode the association. Brain will be
...
from brain.safety.diagnostics.fault_model import Fault, FaultSeverity
...
class FaultCommunicator:
    """
    Manages the social communication of faults to humans.
    ...
    def on_conversation_start(self, person_id: Optional[str] = None):
        """Called when a human begins speaking to Brain."""
    ...
    def _deliver(self, comm: PendingCommunication):
        ...
        if self._bus:
            from brain.bus.neural_bus import T
    ...
    def _action_prompt(self, fault: Fault, bond: float) -> str:
        ...
        if fault.severity == FaultSeverity.DEGRADED:
            return ""  # let Brain handle degraded silently
        if fault.severity == FaultSeverity.IMPAIRED:
