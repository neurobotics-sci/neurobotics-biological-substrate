"""
brain/safety/diagnostics/fault_model.py — Brain v10000

Fault Taxonomy and Data Model
==============================

The complete vocabulary of things that can go wrong with Brain,
...
The key distinction:
  ...
  Brain fault handling: detect fault → feel it → protect → adapt → communicate → seek repair

The "feel it" step is not metaphorical. A fault in Brain's left arm
...
modulates PFC decision-making. The fault *changes how Brain feels*,
...
=====================
  ...
  TIER 4 — SAFETY CRITICAL (NALB + Vagus involvement)
    Fault threatens human safety or Brain structural integrity.
    NALB social mode override. Possible vagus nerve involvement.
    Example: galvanic barrier breach — halt all motion, call for help

AUTOPOIESIS NOTE:
=================
This system implements the *diagnostic half* of autopoiesis.
Brain cannot replace its own hardware. But it can:
...
it organises repair processes. Brain does the same.
"""
...
@dataclass
class Fault:
    """A detected fault in Brain's systems."""
    fault_id:       str
    category:       str
    ...
    def human_message(self) -> str:
        """Natural language fault report for social communication."""
        severity_prefix = {
...
class DiagnosticReport:
    """Snapshot of Brain's overall health."""
    timestamp_ns:      int = field(default_factory=time.time_ns)
