"""
brain/safety/diagnostics/self_diagnostic.py — Brain v10000
...
FAULT MEMORY (AMYGDALA INTEGRATION):
  ...
  Brain will become cautious about that context proactively —
...
FAULT_HISTORY_PATH = Path("/opt/brain/data/fault_history.json")
...
class SelfDiagnosticEngine:
    ...
    Central nervous system for Brain's bodily self-awareness.
    """
    ...
    def _init_channels(self):
        ...
        for node in ["pfc_l", "social", "hippocampus", "amygdala", "visual"]:
    ...
    def raise_fault(self, category: str, severity: FaultSeverity,
        ...
        """Raise a fault from external code (spinal, social, etc.)."""
        with self._lock:
            self._fault_counter += 1
            fault = Fault(
                fault_id=f"DIAG-{self._fault_counter:05d}",
