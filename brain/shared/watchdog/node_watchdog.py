"""
brain/shared/watchdog/node_watchdog.py
...
Also: WatchdogSupervisor runs on blueraven/manager and monitors all 25 nodes.
...
class NodeWatchdog:
    """
    Embedded watchdog for one Brain node process.
    ...
    def __init__(self, node_name: str, bus, pid: int = None):
        ...
        self._health_log = Path(f"/var/log/brain/{node_name}_watchdog.log")
...
class WatchdogSupervisor:
    ...
    NODES = {
        "pfc_l":       "192.168.1.10", "pfc_r":        "192.168.1.11",
    ...
    SSH_KEY = Path.home() / ".ssh/brain_id_ed25519"
    ...
    def _restart_node(self, node_name: str):
        ...
        try:
            subprocess.run(
                ...
                f"'systemctl restart brain-{node_name}'",
